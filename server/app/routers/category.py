...
def list_categories(
    ...
    """
    categories = crud.get_categories(db, user_id=user_id, type_=type)
...
def create_category(
    ...
    try:
        new_cat = crud.create_category(
            db, user_id=user_id, name=category.name, type_=category.type,
...
def update_category(
    ...
    try:
        ...
        updated = crud.update_category(db, category_id, user_id=user_id, **data)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    if not updated:
...
def delete_category(
    ...
    """
    try:
        if not crud.delete_category(db, category_id, user_id=user_id, cascade=cascade):
            raise HTTPException(status_code=404, detail="分类不存在")
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
